...
%        results.vcv   = White robust VCV matrix. added 15jan05 -AJP.
%   	 results.Fstat = Fstat joint test that all parameters except the first (ie, the constant) equal zero (AJP 10mar10)
%   	 results.Fstatpval	 = p-value on joint test that all parameters except the first (ie, the constant) equal zero (AJP 10mar10)
%        results.se = standard errors on coefficient estimates  (AJP 10mar10)
% --------------------------------------------------
...
results.vcv = xpxia;             % added 15jan05. -ajp
results.se = sqrt(diag(xpxia));  % added 10mar10. -ajp
tmp = sqrt(diag(xpxia));
...
if length(results.beta)>1  % added 10mar10
    results.Fstat = results.beta(2:end)'*inv(results.vcv(2:end,2:end))*results.beta(2:end);
    results.Fstatpval = 1-chi2cdf(results.beta(2:end)'*inv(results.vcv(2:end,2:end))*results.beta(2:end),length(results.beta-1));
else
    results.Fstat=[];
    results.Fstatpval=[];
end
